...
const MessageBubble = ({ message, currentUserId }) => {
    ...
    return (
        <div className={`msg-message-bubble-wrapper-ngo ${senderType}`}>
            <div className="msg-message-bubble-ngo">
                <p>{message.content}</p>
...
function MsgForNgo() {
    ...
    return (
        <div className="msg-container-ngo">
            
            <div className="msg-left-ngo">
                <h1>Messages</h1>
                <div className="msg-search-ngo">
                    <FaSearch className="msg-search-icon" />
                    <input placeholder="Search conversations" className="msg-sear-ngo"></input>
                </div>
                
                <div className="msg-chat-list-ngo">
                    {error && <p className="msg-no-chats error-message">{error}</p>}
                    
                    {isLoading && conversations.length === 0 && <p className="msg-no-chats-ngo">Loading chats...</p>}
                    ...
                    {!isLoading && conversations.length > 0 ? (
                       ...
                       !isLoading && !error && <p className="msg-no-chats-ngo">No recent chats</p>
                    )}
                </div>
            </div>
                ...
                {selectedChat ? (
                    <>
                        <div className="msg-top-ngo">
                            ...
                            <div className="msg-header-info-ngo">
                                <h1>{selectedChat.name}</h1>
                                <p>{selectedChat.role}</p>
                            </div>
                            <button className="msg-options-btn-ngo">
                                <BsThreeDotsVertical />
                            </button>
                        </div>
                        
                        <div className="msg-body-ngo" ref={chatBodyRef}>
                            {messages.map(msg => (
                                <MessageBubble key={msg._id || msg.id} message={msg} currentUserId={myId} />
                            ))}
                        </div>
                        
                        <form className="msg-msging-ngo" onSubmit={handleSendMessage}>
                            <button type="button" className="msg-plus" name="plus">
                                <BsPlusCircle />
                            </button>
                            <input 
                                className="msg-input-ngo" 
                                name="input" 
                                placeholder="Type your message"
                                value={newMessage}
                                onChange={(e) => setNewMessage(e.target.value)}
                            />
                            <button type="submit" className="msg-send-ngo">
                                <FiSend />
                            </button>
                        </form>
                    </>
                ) : (
                    <div className="msg-no-chat-selected-ngo">
